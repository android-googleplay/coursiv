...
export function coursivMediaUrl(value?: string) {
  ...
  const configuredBase = (process.env.NEXT_PUBLIC_COURSIV_MEDIA_BASE_URL || FIREBASE_MEDIA_BASE).replace(/\/$/, "");
  return `${configuredBase}/${value.slice(LOCAL_MEDIA_PREFIX.length)}`;
}
