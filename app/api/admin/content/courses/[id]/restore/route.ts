...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const reservation = await reserveAdminMutation(actor, request, `content.course.restore:${id}`);
  ...
  const result = await setAdminCourseArchived(actor, request, id, body.expectedVersion!, false, body.reason.trim());
