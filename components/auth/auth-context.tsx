...
type AuthUser = { id: string; email: string; displayName: string; demo: boolean; providers: string[]; emailVerified: boolean };
...
const demoUser: AuthUser = { id: "demo-hj", email: "hj@lumora.demo", displayName: "HJ", demo: true, providers: ["password"], emailVerified: true };
const debugAdminUser: AuthUser = { id: "debug-admin", email: "admin@coursiv.local", displayName: "Debug Admin", demo: true, providers: ["debug"], emailVerified: true };
const debugAdminRequested = process.env.NEXT_PUBLIC_COURSIV_DEBUG_ADMIN === "true";
const AuthContext = createContext<AuthContextValue | null>(null);

function mapUser(user: User): AuthUser { return { id: user.uid, email: user.email ?? "", displayName: user.displayName ?? user.email?.split("@")[0] ?? "Member", demo: false, providers: user.providerData.map((provider) => provider.providerId), emailVerified: user.emailVerified }; }
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signOut = useCallback(async () => {
    ...
    const client = getFirebaseClient();
    if (client) await firebaseSignOut(client.auth); else { localStorage.setItem(DEMO_AUTH_KEY, "signed-out"); setUser(null); }
  }, []);
  ...
  const value = useMemo(() => ({ user, loading, configured, signIn, signUp, signInWithGoogle, requestPasswordReset, sendVerification, refreshUser, signOut, changeEmail, deleteAccount, getToken }), [changeEmail, configured, deleteAccount, getToken, loading, refreshUser, requestPasswordReset, sendVerification, signIn, signInWithGoogle, signOut, signUp, user]);
