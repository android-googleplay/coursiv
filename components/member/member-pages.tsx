...
import {
  ArrowLeft, ArrowRight, BadgeCheck, Bell, BookOpen, Check, ChevronRight, Clock3,
...
import { calculateStreaks, canCompleteChallengeDay, courseDefinitionPercent, coursePercent, localDateKey, programCompletedCourses, programPercent, weekDateKeys } from "@/lib/learner-state";
...
import { courseEntryHref, courseLessonHref } from "@/lib/course-navigation";
import { ButtonText, useButtonLanguage } from "./button-text";
...
function HorizontalCards({ items, hrefBase }: { items: CatalogItem[]; hrefBase: string }) {
  return <div className="horizontal-cards">{items.map((item, index) => {
    ...
    const href = item.kind === "use-case" ? courseEntryHref(item.id) : `${hrefBase}/${item.id}`;
    return <Link href={href} className="member-course-card" key={item.id}>
...
function dynamicCoursePercent(state: ReturnType<typeof useLearner>["state"], course: CourseDefinition) {
  return courseDefinitionPercent(state,course);
}

function featuredToolItems(items: CatalogItem[]) {
  const priority = ["notion", "claude-deep", "chatgpt-deep", "kling", "canva-ai"];
  const prioritized = priority.map((id) => items.find((item) => item.id === id)).filter((item): item is CatalogItem => Boolean(item));
  const prioritizedIds = new Set(prioritized.map((item) => item.id));
  return [...prioritized, ...items.filter((item) => !prioritizedIds.has(item.id))].slice(0, 5);
}
...
export function DashboardPage({runtimeCourses=[],runtimeItems=[]}:{runtimeCourses?:CourseDefinition[];runtimeItems?:CatalogItem[]}) {
  ...
  const dynamicTools = runtimeItems.filter((item) => item.kind === "tool");
  const dashboardTools = featuredToolItems(dynamicTools.length ? dynamicTools : toolCourses);
  ...
  return (
    <MemberShell>
      <div className="member-content dashboard-content">
        ...
        <SectionTitle title="Explore AI tools" href="/courses/tools" />
        <HorizontalCards items={dashboardTools} hrefBase="/course" />
...
export function CatalogPage({ kind, runtimeItems, runtimeCourses=[] }: { kind: "program" | "tool" | "use-case" | "challenge"; runtimeItems?:CatalogItem[];runtimeCourses?:CourseDefinition[] }) {
  ...
  return <MemberShell title={title}>
    <div className="member-content catalog-page">
      ...
      <div className="filter-chips" role="tablist">{(kind === "challenge" ? ["All", "Completed"] : categories).map((item) => <button key={item} className={category === item ? "active" : ""} onClick={() => setCategory(item)}><ButtonText>{item}</ButtonText></button>)}</div>
      <div className="catalog-grid">{filtered.map((item, index) => {
        ...
        const href=kind==="use-case"?courseEntryHref(item.id):`${base}/${item.id}`;
        return <Link href={href} className="catalog-card" key={item.id}><PlaceholderArt index={index} label={item.title} src={item.image} /><div><strong>{item.title}</strong><span>{count ? `${count} ${kind === "program" ? "courses" : "lessons"}` : `${(item as typeof challenges[number]).days ?? 7} days`}{item.duration ? ` · ${item.duration}` : ""}</span>{status&&<small>{status}</small>}</div><ChevronRight size={20} /></Link>;
      })}</div>
    </div>
  </MemberShell>;
}
...
export function CoursesLandingPage({runtimeCourses=[],runtimeItems=[]}:{runtimeCourses?:CourseDefinition[];runtimeItems?:CatalogItem[]}){
  ...
  const featuredTools=featuredToolItems(toolSource);
  return <MemberShell><div className="member-content courses-landing"><Link className="courses-hero" href={courseEntryHref(course.id)}><PlaceholderArt label={course.title} src={course.image}/><div><small>AI MASTERY</small><h1>{course.title}</h1><span className="courses-hero-action"><ButtonText>Continue learning</ButtonText></span></div><span className="courses-hero-progress"><i style={{width:`${progress}%`}}/></span><b>{completed}/{lessons.length} lessons completed · {progress}%</b></Link><SectionTitle title="Basic Law exam prep"/><div className="basic-law-entries"><Link className="basic-law-entry" href={courseEntryHref("basic-law")}><span><ShieldCheck/></span><div><strong>BLNST 基本法及香港國安法</strong><small>28 lessons · 3.5 hours</small></div><b>Open course <ArrowRight/></b></Link><Link className="basic-law-entry" href={courseEntryHref("basic-law-practice")}><span><BookOpen/></span><div><strong>BLNST 310 題 Practice Bank</strong><small>1 lesson · Self-paced</small></div><b>Open course <ArrowRight/></b></Link><Link className="basic-law-entry" href={courseEntryHref("basic-law-mocks")}><span><Clock3/></span><div><strong>BLNST 30 分鐘模擬試</strong><small>4 lessons · 2 hours</small></div><b>Open course <ArrowRight/></b></Link></div><SectionTitle title="Explore AI tools" href="/courses/tools"/><HorizontalCards items={featuredTools} hrefBase="/course"/><SectionTitle title="Discover AI use cases" href="/use-cases"/><HorizontalCards items={useCaseSource.slice(0,5)} hrefBase="/use-cases"/></div></MemberShell>;
}
...
export function UseCaseDetailPage({runtimeItem,runtimeCourse}:{runtimeItem?:CatalogItem;runtimeCourse?:CourseDefinition}={}){
  ...
  const lessons=useCase?allCourseLessons(course):[];
  if(!useCase)return <MissingPage kind="Use case"/>;
  return <MemberShell title="AI Use Case"><div className="member-content program-page"><PlaceholderArt label={useCase.title}/><span className="program-kicker">PRACTICAL LEARNING PATH</span><h1>{useCase.title}</h1><p>Build a practical workflow you can reuse in real projects while keeping human review in control.</p><div className="program-course-list">{lessons.map((lesson,index)=><button key={lesson.id} onClick={()=>router.push(courseLessonHref(useCase.id,lesson.id))}><i className="module-number">{index+1}</i><span><strong>{lesson.title}</strong><small>Lesson · practical exercise</small></span><ChevronRight/></button>)}</div><button className="member-primary" disabled={!lessons.length} onClick={()=>lessons[0]&&router.push(courseLessonHref(useCase.id,lessons[0].id))}><ButtonText>{`Start ${useCase.title}`}</ButtonText></button></div></MemberShell>
}
...
function saveLocalOutbox(type:"support"|"feedback",message:string,email?:string) {
  ...
  localStorage.setItem(key,JSON.stringify([...current,{id:browserUuid(),type,message,email:email??null,createdAt:new Date().toISOString(),status:"waiting-for-server"}]));
}
...
export function SettingsPage() {
  const router = useRouter(); const auth = useAuth(); const { state, updatePreference } = useLearner(); const {setLanguage}=useButtonLanguage(); const [action, setAction] = useState<string | null>(null); const [status,setStatus]=useState(""); const [billingReady,setBillingReady]=useState<boolean|null>(null); const [pushReady,setPushReady]=useState<boolean|null>(null);
  const rows = [
